...
### ステップ1: Unity Editorを完全に再起動

**重要：** 新しいスクリプトを追加したので、完全な再起動が必要です。

```
1. Unity Editorが開いている場合は完全に閉じる
2. Unity Hub を起動
3. Projects リストから "UnityProject" を選択
...
2. 完了するまで待つ（初回は5-10分、再コンパイルは1-2分）
3. Console (Window > General > Console) を開く
4. エラー（赤いアイコン）がないことを確認
```

**⚠️ エラーがある場合：**
- エラーメッセージをダブルクリックして詳細を確認
- `Assets/Scripts/Editor` フォルダ内のスクリプトが原因の場合は報告してください
...
```
1. Tools > Kokoro no Boukensha > 【テスト】メニュー表示確認 をクリック
2. ダイアログが表示されれば成功！
3. すべてのツールが利用可能です
```
