...
def test_story_dag_implementation():
    """8.1 ストーリーDAG?"""
    print("? Testing Story DAG Implementation (8.1)...")
    ...
    try:
        ...
        print(f"  ? Story DAG app initialized: {story_dag_app.title}")
        print(f"  ? Sample data loaded: {len(db.chapters)} chapters, {len(db.nodes)} nodes, {len(db.edges)} edges")
        print(f"  ? DAG validation: cycles={has_cycles}, isolated={len(isolated_nodes)}")
        
        return True
    except Exception as e:
        print(f"  ? Story DAG test failed: {e}")
        return False

def test_ai_story_generation():
    """8.2 GPT-4oストーリー"""
    print("? Testing AI Story Generation (8.2)...")
    ...
    try:
        ...
        print(f"  ? AI Story app initialized: {app.title}")
        print(f"  ? DeepSeek R1 client: {deepseek_client.model}")
        print(f"  ? Prompt templates: {len(prompt_manager.templates)}")
        print(f"  ? Safety filter: {len(safety_filter.harmful_patterns)} patterns")
        print(f"  ? Fallback templates: {len(fallback_system.templates)} categories")
        
        return True
    except Exception as e:
        print(f"  ? AI Story Generation test failed: {e}")
        return False

def test_task_story_integration():
    """8.3 リスト"""
    print("? Testing Task-Story Integration (8.3)...")
    ...
    try:
        ...
        print(f"  ? Integration app initialized: {app.title}")
        print(f"  ? ServiceIntegration class: functional")
        print(f"  ? Story choice conversion: {choice_hook.choice_text} -> {task_data['task_type']}")
        print(f"  ? Task-story hooks: implemented")
...
async def test_integration_flow():
    ...
    try:
        ...
        print("  ? Story choice -> Task creation: functional")
        print("  ? Task completion -> Story sync: functional")
...
def test_api_endpoints():
    ...
    try:
        ...
        print(f"  ? Story DAG endpoints: {len(story_dag_routes)} routes")
        print(f"  ? AI Story endpoints: {len(ai_story_routes)} routes")
        print(f"  ? Integration endpoints: {len(integration_routes)} routes")
        ...
        for service_name, endpoints in key_endpoints.items():
            ...
            for endpoint in endpoints:
                ...
                if found:
                    ...
                    print(f"    ? {service_name}: {endpoint} NOT FOUND")
...
def test_requirements_compliance():
    ...
    requirements_check = {
        "2.1 AI?3.5?": "DeepSeek R1 client with timeout=30.0 (mock: 0.5s)",
        "2.2 GPT-4o?JSONストーリー": "DeepSeek R1 integration with JSON validation",
        "2.3 CHAPTER > NODE > EDGE?": "Story DAG with hierarchical structure",
        "2.4 ?": "detect_cycle() and auto_merge_isolated_nodes()",
        "1.4 ストーリーreal_task_id?": "StoryChoiceHook with real_task_id/habit_tag",
        "5.5 タスク": "TaskCompletionHook with story progression"
    }
...
def main():
    ...
    print("=" * 70)
    print("? Task 8: AIストーリー - Complete Verification")
    print("=" * 70)
    ...
    if passed == total:
        print("\n? TASK 8 COMPLETE!")
        print("? All subtasks successfully implemented:")
        print("   ? 8.1 ストーリーDAG?")
        print("   ? 8.2 GPT-4oストーリー")
        print("   ? 8.3 リスト")
        print("\n? AIストーリー")
        print("\n? Requirements satisfied:")
        print("   ? ? 1.4: ストーリーreal_task_id/habit_tagの")
        print("   ? ? 2.1: AI?GPT-4o?")
        print("   ? ? 2.2: JSONストーリー")
        print("   ? ? 2.3: CHAPTER > NODE > EDGE?")
        print("   ? ? 2.4: ?")
        print("   ? ? 5.5: タスク")
